...
app.post('/api/admin/ai-chat', adminAuth, async (req, res) => {
  ...
  const systemPrompt =
    ...
    `PLATFORM TECH STACK (answer accurately when asked): SignFlow runs on Node.js 20 + Express 4, PostgreSQL 16 (AWS Lightsail Managed DB), Amazon S3 for file/image storage, PM2 process manager, Nginx reverse proxy, Auth0 OIDC for admin authentication. The AI assistant (you) is powered by Anthropic Claude (claude-haiku-4-5). VECTOR SEARCH: SignFlow uses Voyage AI (model: voyage-3-lite) for semantic vector embeddings on the Help documentation. At server startup every Help section is chunked, tagged, and embedded into a 1024-dim float32 vector via the Voyage AI /v1/embeddings API. On each AI chat request the user's message is also embedded by Voyage AI, then cosine-similarity ranked against all Help chunks, and the top-4 most relevant sections are injected into the AI system prompt as live context — that is the "Relevant SignFlow Documentation" block you see below. If VOYAGE_API_KEY is not set the system falls back to keyword matching. This means the assistant already has vector-powered RAG (Retrieval-Augmented Generation) built in for Help content.\n\n` +
